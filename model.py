...
class Model(object):
    ...
    def save(self):
        ...
        for name, param in self.components.items():
            ...
            joblib.dump(param_values, param_path)
    ...
    def reload(self):
        ...
        for name, param in self.components.items():
            param_path = os.path.join(self.model_path, "%s.mat" % name)
            param_values = joblib.load(param_path)
            if hasattr(param, 'params'):
    ...
    def build(self,
        ...
        print 'Compiling...'
        if training:
            updates = Optimization(clip=5.).get_updates(lr_method_name, cost, params, **lr_method_parameters)
