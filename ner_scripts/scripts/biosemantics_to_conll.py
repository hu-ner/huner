...
for directory in dirs:
    files += [os.path.join(directory, file) for file in os.listdir(os.path.join(args.input_dir, directory)) if
              file[-4:] == '.ann']
        ...
        with open(os.path.join(args.input_dir, file[:-4] + '.txt'), 'r') as f_txt:
            ...
            for line in f_ann:
                ...
                if (tmp_sentences[id][start:end] != text[o_start:o_end].replace('\n', ' ') or
                        tmp_sentences[id][start:end] != line[4]):
                    file_is_dirty = True
                    break
                else:
                    tmp_entities[id] += [(start, end)]

            if not file_is_dirty:
